...
### Structure
- `src/agent_skill_adapter/envspec/` — descriptions and their comparison; `rules.py`, `assets.py` and `convert.py` sit beside it and depend on it, never the reverse; `cli/main.py` is a typer app with `--version` and the one `convert` command.
- `specs/<vendor>/<environment>-<version>.yaml` — hand-written descriptions; `specs/gaps/` and `specs/schema/` — generated, never hand-edited. `rules/claude-code-to-antigravity-1.0.yaml` — the translation rules, versioned apart from the descriptions.
- `docs/adr/0001..0009` hold why each rule is what it is; `docs/converting-a-real-skill.md` records three real `convert` runs and `docs/live-acceptance.md` the procedure for asking the live Antigravity CLI what it loaded — their commands need `--allow-stale` from 2026-10-14, when the descriptions fall due for a re-check.
...
- With `--out`, every destination comes from the target's `layout` (`skills.*`, `skill.file`, `skill.dir.*`, `agents.*`, `rules.project` at either level, `hooks.*`) with `<skill-name>`, `<workspace-root>` and `~` expanded; a command has no entry anywhere and is written nowhere, and a rule file gains the `trigger` its target loads it by — the value from the description's closed set, the decision to add it from the translation rules — no path is ever spelled in the converter. A kind the target names no root for is written nowhere and earns a row plus `lossy` — never a refusal, which would cost the rest of the set its rows; a path no description declares is staged under `--out` beside the part it came from and never in a root the target scans for entities. The whole plan is checked for collisions and for escapes out of `--out` before the first byte is written, an `undecidable` run assembles nothing, and a hook entry is staged beside the skill for a person to merge by hand.
